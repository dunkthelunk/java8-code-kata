...
public class Exercise8Test extends ClassicOnlineStore {
    ...
    public void havingEnoughMoney() {
        ...
        Predicate<Customer> havingEnoughMoney = c -> c.getBudget() >=
                c.getWantToBuy()
                        .stream()
                        .map(Item::getName)
                        .mapToInt(iname -> onSale.stream().filter(onsaleitem -> onsaleitem.getName().equals(iname)).mapToInt(Item::getPrice).min().orElse(0))
                        .sum();
